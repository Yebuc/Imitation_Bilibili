# 一个在线的视频投稿分享交流平台
参考哔哩哔哩，抖音等视频平台，从数据库、网站架构设计、开发并实现用户、权限、动态、视频、弹幕等核心功能
...
````md
发布订阅模式中间有个代理人的角色，而观察者模式是只是生产者和消费者的。发布订阅模式的可扩展性较高,在多系统的数据交互上可以拥有较高的性能和灵活性。
````
...
```md
原理：截取视频帧截图，将截图内容进行人像与背景的分割，将人像分割出来；最后将人体轮廓图转换为黑白剪影图，黑色的为背景，白色的为人像剪影；前端通过黑白剪影图覆盖原视频形成遮挡弹幕的效果。--->弹幕在经过黑色背景部分正常显示，在经过白色人像剪影部分将被遮挡不显示。

步骤：截取视频帧、人像分割、图转换--->黑白剪影图、覆盖

其中，截取视频使用JavaCV提供的功能库，人像分割及图像转换调用百度提供的免费开放API来实现
观看百度API官方文档---->其中有一个人像分割api调用接口使用示例
```
